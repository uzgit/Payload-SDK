...
T_DjiReturnCode LiveviewSample::joshua_start_file_stream(CameraImageCallback callback, void *userData)
{
    cout << "starting file stream..." << endl;
    ...
    // i think you can start by piping data to the decoder from here -- obviously not completely sure though

//    auto deocder = streamDecoder.find(DJI_LIVEVIEW_CAMERA_POSITION_FPV);
//    if ((deocder != streamDecoder.end()) && deocder->second) {
//        cout << "initializing decoder..." << endl;
//        deocder->second->init();
...
T_DjiReturnCode LiveviewSample::joshua_stop_file_stream()
{
	cout << "stopping file stream..." << endl;
    T_DjiReturnCode returnCode;
